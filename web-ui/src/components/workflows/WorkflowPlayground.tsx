...
export function WorkflowPlayground({
  ...
  useEffect(() => {
    ...
    const load = () =>
      void api
        .delegation(selectedId)
        .then((payload) => {
          if (!cancelled) setDetail(payload);
        })
        .catch((error: Error) => {
          if (!cancelled)
            setLoadError(error.message || "Could not load this workflow.");
        })
        .finally(() => {
    ...
    <section className="workflow-page" aria-label="Workflow playground">
      <aside className="workflow-rail" aria-label="Workflows">
        <div className="workflow-rail-header">
          ...
          <button type="button" className="workflow-new" onClick={selectDraft}>
            ＋ New
          </button>
        </div>
        <p className="workflow-rail-copy">
          Compose agents, watch their state, and inspect each bot’s output in
          one place.
        </p>
        <div className="workflow-list" role="list">
          <button
            type="button"
            className={`workflow-list-item draft${!selectedId ? " selected" : ""}`}
            onClick={selectDraft}
          >
          ...
          {[...plans].reverse().map((plan) => (
            <button
              key={plan.id}
              type="button"
              className={`workflow-list-item${selectedId === plan.id ? " selected" : ""}`}
            ...
            >
              ...
              <span>
                ...
                <small>
                  {plan.status.replaceAll("_", " ")} · {plan.max_fanout ?? 1}{" "}
                  parallel · depth {plan.max_depth ?? 1}
                </small>
              </span>
            </button>
          ))}
          {plans.length === 0 && (
            <p className="workflow-list-empty">
              No saved workflows yet. Build the first one here.
            </p>
          )}
        </div>
      </aside>
      <div className="workflow-stage">
        <header className="workflow-stage-header">
          <div>
            <p className="eyebrow">
              {selectedPlan ? "Saved workflow" : "New workflow"}
            </p>
          ...
          <div className="workflow-stage-state">
            {loading
              ...
              : loadError ||
                (selectedPlan
                  ? `${selectedPlan.status.replaceAll("_", " ")} · outputs below`
                  : "Draft · not started")}
          </div>
        </header>
        {loadError ? (
          ...
          <WorkflowStudio
            ...
            onStartPlan={(id) => {
              if (selectedPlan && selectedPlan.id === id) onStart(selectedPlan);
            }}
            onCancelPlan={(id) => {
        ...
        {selectedPlan && !loading && terminal.has(selectedPlan.status) && (
          <p className="workflow-terminal-note">
            This workflow is complete. Its graph and recorded bot outputs remain

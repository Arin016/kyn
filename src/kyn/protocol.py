...
def _tool_identity(payload: dict[str, Any], namespace: str) -> tuple[str, str]:
    if namespace == "kiro":
        meta = payload.get("_meta") if isinstance(payload.get("_meta"), dict) else {}
        trusted = meta.get(namespace) if isinstance(meta.get(namespace), dict) else {}
        return (
            str(trusted.get("toolName") or ""),
            str(trusted.get("mcpServerName") or ""),
        )
    return (str(payload.get("name") or ""), "")


def parse_update(message: dict[str, Any], identity_namespace: str = "kiro") -> list[Event]:
    ...
    if kind in {"tool_call", "tool_call_update"}:
        ...
        tool_name, mcp_server_name = _tool_identity(update, identity_namespace)
        return [
            Event(
...
def parse_permission(message: dict[str, Any], identity_namespace: str = "kiro") -> Event:
    ...
    if not meta and isinstance(params.get("_meta"), dict):
        meta = params["_meta"]
    tool_name, mcp_server_name = _tool_identity(
        {"_meta": meta, "name": tool_call.get("name")}, identity_namespace
    )
    return Event(
        kind="permission",
